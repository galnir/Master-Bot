...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (
      ...
      return interaction.reply(`:x: Sorry **${number} ${unit}** is too long`);
    }
    ...
    if (timer * 60000 >= 3600000) {
      //only save if reminder is more than 1 hour
      await Reminder.create(reminderObject);
    }

    interaction.member.reminders.push(reminderObject);

    interaction.reply(`:white_check_mark: Reminder is set: ${reminderMessage}`);
